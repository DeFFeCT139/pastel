...
function Card({card}) {
  ...
  const setLike = () => {
    if (JSON.parse(localStorage.getItem('like')).length === 0) {
      localStorage.setItem('like', JSON.stringify([card]))
    } else {
      let loc = JSON.parse(localStorage.getItem('like'))
      loc.unshift(card)
      localStorage.setItem('like', JSON.stringify(loc))
    }
  }

  const setBascet = () => {
    if (JSON.parse(localStorage.getItem('bascet')).length === 0) {
      localStorage.setItem('bascet', JSON.stringify([card]))
    } else {
    ...
    <div className="card">
      ...
      <div className="card-content">
        ...
        <div className="card-content-priseBbtn">
          ...
          <div onClick={setBascet} className="card-content-btn">В КОРЗИНУ</div>
        </div>
      </div> 
      <div onClick={setLike} className="card-live">
